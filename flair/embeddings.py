...
class Embeddings(torch.nn.Module):
    ...
    '''
    def embed(self, sentences: Union[Sentence, List[Sentence]], data_type) -> List[Sentence]:
        ...
        if not everything_embedded or not self.static_embeddings:
            ...
            '''
            # fix start
            self._add_embeddings_internal(sentences, data_type)
            # fix end
        return sentences
...
class StackedEmbeddings(TokenEmbeddings):
    ...
    '''
    def embed(self, sentences: Union[Sentence, List[Sentence]], data_type, static_embeddings: bool = True):
        # if only one sentence is passed, convert to list of sentence
        if type(sentences) is Sentence:
            sentences = [sentences]
        ...
        for embedding in self.embeddings:
            ...
            '''
            # fix start
            embedding.embed(sentences, data_type)
            # fix end
...
class WordEmbeddings(TokenEmbeddings):
    ...
    def _add_embeddings_internal(self, sentences: List[Sentence]) -> List[Sentence]:
    '''
    def _add_embeddings_internal(self, sentences: List[Sentence], data_type) -> List[Sentence]:
...
class FlairEmbeddings(TokenEmbeddings):
    ...
    def __init__(self, model: str, use_cache: bool = False, cache_directory: Path = None, chars_per_chunk: int = 512):
        ...
        '''
        # fix start
        embedded_dummy = self.embed(dummy_sentence, 'dummy')
        # fix end
        self.__embedding_length: int = len(embedded_dummy[0].get_token(1).get_embedding())
    ...
    def _add_embeddings_internal(self, sentences: List[Sentence], data_type) -> List[Sentence]:
        ...
        with torch.no_grad():
            ...
            for i, sentence in enumerate(sentences):
                ...
                offset_forward: int = extra_offset #extra_offsetのデフォルトは1
                offset_backward: int = len(sentence_text) + extra_offset #extra_offsetのデフォルトは1
                #print('')
                for token in sentence.tokens:
                    #print('batch len:{}'.format(self.len_batches))
                    # fix start
                    if data_type == 'train':
                        #print('{} {}'.format(token, token.tags))
                        #import pdb; pdb.set_trace()
                        token_label = str(token.tags['ner']).split(' ')[0][0]
                        if token_label == 'S':
                            #NE前のスペースベクトルを抽出
                            if self.is_forward_lm:
                                ex_offset = offset_forward - 1
                            #NE後のスペースベクトルを抽出
                            else:
                                ex_offset = offset_backward - len(token.text) - 1
                            #print('len(hx_stacked):{}  ex_offset:{}'.format(len(hx_stacked), ex_offset))
                    ...
                    embedding = all_hidden_states_in_lm[offset, i, :] #torch.Size([2048])
