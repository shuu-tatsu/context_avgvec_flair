...
class SequenceTagger(flair.nn.Model):
    ...
    def forward_loss(self, sentences: Union[List[Sentence], Sentence],  sort=True) -> torch.tensor:
        ...
        '''
        # fix start
        features, lengths, tags = self.forward(sentences, 'train', sort=sort)
        # fix end
    ...
    def forward_labels_and_loss(self, sentences: Union[List[Sentence], Sentence],
        ...
        with torch.no_grad():
            ...
            '''
            # fix start
            feature, lengths, tags = self.forward(sentences, 'eval', sort=sort)
            # fix end
    ...
    def forward(self, sentences: List[Sentence], data_type, sort=True):
        self.zero_grad()
        '''
        self.embeddings.embed(sentences)
        '''
        # fix start
        self.embeddings.embed(sentences, data_type)
        # fix end
